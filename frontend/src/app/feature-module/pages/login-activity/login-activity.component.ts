...
export class LoginActivityComponent implements OnInit{
  ...
  public loginActivity : Array<providersDeviceManagement> = []
  ...
  private getTableData(): void {
    ...
    this.data.getLoginActivity().subscribe((data: apiResultFormat) => {
      this.totalData = data.totalData;
      data.data.map((res: providersDeviceManagement, index: number) => {
        const serialNumber = index + 1;
        if (index >= this.skip && serialNumber <= this.limit) {
          res.id = serialNumber;
          this.loginActivity.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<providersDeviceManagement>(this.loginActivity);
  ...
  public searchData(value: string): void {
    ...
    this.loginActivity = this.dataSource.filteredData;
  }

  public sortData(sort: Sort) {
    const data = this.loginActivity.slice();

    if (!sort.active || sort.direction === '') {
      this.loginActivity = data;
    } else {
      this.loginActivity = data.sort((a, b) => {         
